...
export class GlobalSearchSh extends Component {
    ...
    async setup() {
        super.setup()
         // todo
        /*onWillStart(async () => {
            show_company = await user.hasGroup('base.group_multi_company');
        });*/
        this.orm = useService("orm");
        this._search_def = $.Deferred();
        this.show_company = show_company
        this.sh_enable_gloabl_search_mode = session.sh_enable_gloabl_search_mode !== false
        onMounted(() => {
   ...
   async _searchData() {
        var searchInput = this.$search_input || $(".sh_search_input input.usermenu_search_input");
