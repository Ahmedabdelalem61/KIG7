...
class Http(models.AbstractModel):
    ...
    def session_info(self):
        ...
        #
        # token_ids = request.env['res.users.apikeys'].search([('user_id', '=', self.env.user.id)])
        # if token_ids:
        #     token= token_ids[-1].name
        # else:
        token = self.env['res.users.apikeys'].sudo()._generate(
            None,
            'Highnox Dashboard',
            None,
        )
        base_url = self.env['ir.config_parameter'].sudo().get_param(
            'floor_api.hig_base_url', default='/'
        )
        info['customer_dash_token'] = token
        info['customer_dash_url'] = base_url+"/highnox"
        info['customer_dash_user'] = self.env.user.name
        return info
