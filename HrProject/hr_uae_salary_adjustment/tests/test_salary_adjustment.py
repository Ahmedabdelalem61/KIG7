...
class TestHrUaeSalaryAdjustment(TransactionCase):
    ...
    def test_range_pushes_to_matching_payslips(self):
        ...
        ded_line = self.slip.line_ids.filtered(lambda line: line.code == "ADJ_DED_LINE")
        net_line = self.slip.line_ids.filtered(lambda line: line.code == "NET")
        self.assertTrue(ded_line)
        self.assertEqual(ded_line.category_id.code, "DED")
        self.assertAlmostEqual(ded_line.total, -50.0)
        self.assertAlmostEqual(net_line.total, 4950.0)
